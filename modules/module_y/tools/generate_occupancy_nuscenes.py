...
def points_in_boxes_cpu(points, boxes):
    ...
    half_sizes_exp = half_sizes.unsqueeze(1)

    point_offset = points_exp - centers_exp

    inside_flags = (point_offset.abs() <= half_sizes_exp).all(dim=-1)
    return inside_flags


def main(nusc, nuscenesyaml, args, config):
...
if __name__ == '__main__':
    ...
    parse.add_argument('--save_path', type=str, default='data/set/nuscenes/occupancy_gt/')
    parse.add_argument('--dataroot', type=str, default='data/set/nuscenes/')
    parse.add_argument('--label_mapping', type=str, default='config/nuscenes.yaml')
    ...
    main(nusc, nuscenesyaml=nuscenesyaml, args=args, config=config)
